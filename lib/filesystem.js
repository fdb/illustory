 ...
 *     movies/        — movie scene video files (.mp4, .webm)
 *     sounds/        — looping scene background audio (.mp3, .ogg, .m4a, .wav)
 */

const IMAGE_EXTENSIONS = new Set(['png', 'jpg', 'jpeg', 'webp', 'gif', 'svg']);
const VIDEO_EXTENSIONS = new Set(['mp4', 'webm']);
const AUDIO_EXTENSIONS = new Set(['mp3', 'ogg', 'm4a', 'wav', 'aac', 'flac']);
const ASSET_FOLDERS = {
  backgrounds: IMAGE_EXTENSIONS,
  objects: IMAGE_EXTENSIONS,
  highlights: IMAGE_EXTENSIONS,
  movies: VIDEO_EXTENSIONS,
  sounds: AUDIO_EXTENSIONS,
};
...
function withCanvasDefaults(story, fallbackW, fallbackH) {
  return {
    ...
    scenes: (story.scenes ?? []).map(scene => scene.type === 'movie' ? {
      ...scene,
 ...
 * Returns { backgrounds: [...], objects: [...], highlights: [...], movies: [...], sounds: [...] }
 */
export async function scanImages(dirHandle) {
  const result = { backgrounds: [], objects: [], highlights: [], movies: [], sounds: [] };
  let imagesDir;
  try {
  ...
  for (const [folder, allowedExtensions] of Object.entries(ASSET_FOLDERS)) {
    try {
